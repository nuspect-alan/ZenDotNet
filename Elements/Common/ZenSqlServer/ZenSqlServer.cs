...
namespace ZenSqlServer
{
#if NETCOREAPP2_0
    public class ZenSqlServer
#else
    public class ZenSqlServer : IZenAction //, IZenCallable
#endif
    {
        #region Fields
        #region _script
    ...
    */
        ...
        {
            ExecuteSql(elements, element, ParentBoard);
        }
        #endregion
        #endregion
        #endregion
        ...
        void ExecuteSql(Hashtable elements, IElement element, IGadgeteerBoard ParentBoard)
        {
            ...
            try
            {
                ...
                string connectionString = "Server=" + ZenCsScriptCore.Decode(element.GetElementProperty("SERVER")) + ";Database=" + element.GetElementProperty("DATABASE") + ";" + "User Id=" + element.GetElementProperty("UID") + ";" + "Password=" + element.GetElementProperty("PASSWORD") + ";";
                                ...
                                if (!string.IsNullOrEmpty(element.GetElementProperty("SQLSERVER_PARAMETERS")))
                                {
                                    foreach (string s in element.GetElementProperty("SQLSERVER_PARAMETERS").Split('¨'))
                                    {
                                        if (!string.IsNullOrEmpty(s))
                                            AddSqlServerParam(s.Split('°')[0], s.Split('°')[1], elements, element, command, ParentBoard);
                                    }
                                }
